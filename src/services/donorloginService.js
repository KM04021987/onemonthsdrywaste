...
let handleLogin = (phone, password) => {
    console.log('donorloginservice: handleLogin')
...
let findUserByPhone = (phone) => {
    console.log('donorloginService: findUserByPhone')
...
let findUserByAccount = (account) => {
    console.log('donorloginService: findUserByAccount')
    return new Promise((resolve, reject) => {
        try {
            ibmdb.open(connStr, function (err, conn) {
                if (err) throw err;
                conn.query("SELECT * FROM "+process.env.DB_SCHEMA+".donor_info WHERE account=? with ur;", [account], function(err, rows) {
                    if (err) {
                        reject(err)
                    }
                    let user = rows[0];
...
let comparePassword = (password, user) => {
    console.log('donorloginService: comparePassword')
    return new Promise(async (resolve, reject) => {
        try {
